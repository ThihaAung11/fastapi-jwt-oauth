...
from app.core import schemas, db, config
from app.infrastructure import  crud, models
...
def create_refresh_token(data: dict, expires_delta: timedelta | None = None):
    ...
    if expires_delta:
        ...
        expire = datetime.utcnow() + timedelta(minutes=config.settings.REFRESH_TOKEN_EXPIRE_MINUTES)
    to_encode.update({"exp": expire})
    encoded_jwt = jwt.encode(to_encode, config.settings.SECRET_KEY, algorithm=config.settings.ALGORITHM)
    return encoded_jwt
